...
def save_all_information(folder_path = "code/mk_source_code/web"):
    ...
    for item in data:
        try:
            module_name, module_path = get_module_info(item,                                        
            '/home/noaman/code/muslimkids/code_matrix/code/mk_source_code/web/apps')
            # saving module
            
        except ValueError as ve:
            module_name = 'global_app'
            module_path = '/home/noaman/code/muslimkids/code_matrix/code/mk_source_code/web'
        # adding module for that entry
        module = add_module(session, project.id, module_name, module_path)
        # adding file for that module
        code_file = add_file(session, module.id, item['file_path'], item["file_path"]['code'])
        # adding import
